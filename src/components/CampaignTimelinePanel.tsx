...
import type { CampaignGameRecord, CampaignState, CharacterProfile } from '../types/campaign';
...
function getCharacterDisplayName(character: CharacterProfile) {
  return character.name === '임시 신분증' && character.roleName ? character.roleName : character.name;
}

function formatPlayerCharacter(record: CampaignGameRecord) {
  if (!record.characters.length) return record.players.map((player) => player.name).join(', ');

  return record.characters.map((character) => {
    const playerName = record.players.find((player) => player.id === character.playerId)?.name;
    const characterName = getCharacterDisplayName(character);
    return playerName ? `${playerName}(${characterName})` : characterName;
  }).join(', ');
}
          ...
          <TableBody>
            {records.length ? records.map((record) => (
              <TableRow key={record.id}>
                ...
                <TableCell>{formatPlayerCharacter(record)}</TableCell>
